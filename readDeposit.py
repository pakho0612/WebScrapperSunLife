...
class readDeposit:
    ...
    def readURL(self):
        tables = pd.DataFrame(pd.read_html(self.url, extract_links="all")[7]) ## 7 stores the target insurance data
        colHeader = []
        ...
        print(colHeader)
        deposits = self.readDepositRows(tables.values)
        return deposits

    def rowToClaimEntry(self, row):
        ...
        return ClaimEntry(
                ...
                pdfLink=self.url+row[5][1] ## combine url with relative link
            )
        
    def readDepositRows(self, rows):
        ...
        for row in rows:
            ...
            if not(all(np.equal(row[2], ('', None)))): ## have a valid contract number
                claim = self.rowToClaimEntry(row)
